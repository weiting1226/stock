...
# 股價淨值比低於這個值，代表價格與每股淨值不是同一種股別，而不是「很便宜」。
# 實測案例：BRK-B 收盤 521.80，但資料源給的每股淨值是 505,559——那是 A 股的
# 數字（B 股為 A 股的 1/1500）。算出來 P/B = 0.001，會登上「最便宜」榜首。
# 真正陷入困境的公司大約落在 0.2 上下，0.05 以下只可能是單位對不上。
# 高端不設限：GDDY 1718、CL 315 這類是實打實的——大量庫藏股買回讓帳上
# 股東權益逼近於零，比值自然極高，那不是資料錯誤。
IMPLAUSIBLE_PB_FLOOR = 0.05
