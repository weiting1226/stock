...
**實測結果：2026-08-18 在 GitHub Actions 上被擋下來了。** 三個品牌的查詢
全部收到 `HTTP 403 Forbidden`——是連線層級被拒，不是程式碼原本猜測的
「HTTP 200 但回應包含 `error` 欄位」那種擋法（`fetch_brand()` 兩種都有
處理：連線／HTTP 錯誤在最外層的 try/except 被接住，`error` 欄位則是
另一道專門檢查，回應裡有這個欄位就直接當失敗處理、不嘗試從裡面硬解
資料）。換句話說，程式碼的防呆邏輯運作正常，是蝦皮真的直接擋下了這個
簡單版本的請求。要繞過這層防護需要更進一步的手段（例如帶真實瀏覽器的
cookie／session），目前沒有做——這支爬蟲短期內大概率抓不到任何資料。

**價格單位是另一個還沒被驗證過的假設**（因為連請求都被擋下，根本沒機會
拿到真實回應驗證）。蝦皮這個搜尋端點的價格欄位，公開資料普遍記載是以
「元的十萬分之一」為單位（也就是要除以 100000 才是實際售價），但這個
換算係數本身也只是抄公開資料。如果係數是錯的，換算出來的售價會離譜到
不像一包尿布（例如趨近於 0 或暴衝到幾百萬元）——
