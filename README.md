...
與股票分析無關的獨立小工具：協助在台北晶華酒店 fumée（inline訂位系統，
每月1號00:00開放次月訂位、成功率<1%）開放訂位的瞬間，依你預先設定的
日期/時段/人數優先順序自動嘗試訂位，結果寄信通知。

完整設定步驟、風險與限制說明見 [`fumee_booking/README.md`](fumee_booking/README.md)。
搶位優先順序透過 [`docs/fumee-booking.html`](docs/fumee-booking.html) 網頁表單編輯
（純前端，直接呼叫GitHub API寫回 `fumee_booking/config.json`，不經第三方伺服器）。

**這類工具通常落在訂位平台使用條款的灰色地帶，且無法保證成功率**，
使用前務必先讀完 `fumee_booking/README.md` 的「風險與限制」一節。
...
模組三（fumée搶位）與投資無關，是訂位自動化協助工具，**不保證能成功訂到位，
且使用此類工具有違反訂位平台使用條款的風險**，請自行評估後使用。
