...
| ⑤ | 股票型ETF資金流 | ETF.com 資金流工具 | ✅ 自動（抓取失敗才退回人工值）|
...
- **方向**由淨流的正負決定
- **「大幅」與否**以這個數列**自己的歷史規模**判定，而不是自行發明
  「超過 X 億美元算大幅」這種數字（同 Gate B 用滾動百分位而非寫死絕對值的原則）

規模單位取歷史淨流**絕對值**的中位數——流出期間的規模同樣是規模，只看正值
會讓熊市完全沒有比較基準：
...
刻意**不用百分位當「持平」的門檻**：規格的持平指的是進出接近打平，不是
「跟平常差不多」。若資金流長期都很大，它的後 20% 依然是很大的金額，
把那叫做持平就錯了。

歷史累積在 `docs/data/etf_flows.csv`（來源只給當前值，必須逐日累積才有分布可比）。
**不足 20 筆時只判方向、不判強度**，信心標「中」——方向是確定的，但「大幅與否」
還沒有依據，與其硬給 ±2 不如少說一級。計算時會排除當日自己那筆，否則等於
部分拿自己當比較基準。

ETF.com 是商業網站、頁面由 JavaScript 產生，且開發環境的代理不放行該網域
（無法先行實測）。因此依序嘗試多個位址與兩種解析方式（Next.js 的
`__NEXT_DATA__` 內嵌 JSON → HTML 表格），**每一種都記下實際結果**
（狀態碼、頁面長度、找到的候選標籤），全部失敗才拋出並退回人工填寫值。
若遇到 Cloudflare 或需登入，錯誤訊息會直接點出來。
